...
Two independent memory regressions in Deno **2.9.0** vs **2.8.0**, on a realistic
...
```bash
./bench.sh
# or: IMAGES="denoland/deno:2.8.0 denoland/deno:2.9.0" RELOADS=8 SETTLE=12 ./bench.sh
```
...
| Metric | Deno 2.8.0 | Deno 2.9.0 |
| --- | --- | --- |
| baseline (plain `deno run`, idle) | 115 MB | 561 MB (**~4.9×**) |
| `--watch` growth per reload | ~12 MB | ~472 MB (**~40×**) |
| RSS after 6 reloads | 183 MB | 3388 MB |

Full run (`RELOADS=6`):

```
== denoland/deno:2.8.0 ==          == denoland/deno:2.9.0 ==
  baseline .......  115 MB           baseline .......  561 MB
  reload #0 ......  111 MB           reload #0 ......  558 MB
  reload #1 ......  125 MB           reload #1 ...... 1032 MB
  reload #2 ......  137 MB           reload #2 ...... 1504 MB
  reload #3 ......  149 MB           reload #3 ...... 1975 MB
  reload #4 ......  160 MB           reload #4 ...... 2446 MB
  reload #5 ......  171 MB           reload #5 ...... 2917 MB
  reload #6 ......  183 MB           reload #6 ...... 3388 MB
```

Both `deno --version` report the **same V8 and TypeScript** (`v8 14.9.207.2-rusty`,
`typescript 6.0.3`), so this is Deno's own (Rust-side) npm/node-compat module
handling, not a V8 change.
